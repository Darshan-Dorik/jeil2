...
const CatalogueSubmissions = () => {
  ...
  useEffect(() => {
    setLoading(true);
    axios.get('https://jeil.in/api/catalogue/get-submissions')
      .then((res) => {
  ...
  const updateStatus = async (submissionId, newStatus) => {
    ...
    try {
      ...
      const response = await axios.put(`https://jeil.in/api/catalogue/update-status/${submissionId}`, {
  ...
  const exportToCSV = () => {
    ...
    // Add header row
    csvContent += "Name,Company,Email,Contact Number,City,State,Country,Product,Status,Spam,Date,IP Address\n";
    
    // Add data rows
    filteredSubmissions.forEach(item => {
      const name = item.name ? `"${item.name.replace(/"/g, '""')}"` : "";
      const company = item.companyName ? `"${item.companyName.replace(/"/g, '""')}"` : "";
      ...
      const product = item.productName ? `"${item.productName.replace(/"/g, '""')}"` : "";
      const status = item.followupStatus || "Pending";
      const spam = item.isSpam ? "Yes" : "No";
      const date = item.createdAt ? `"${new Date(item.createdAt).toLocaleString()}"` : "";
      const ip = item.visitorIP ? `"${item.visitorIP}"` : "";
      
      csvContent += `${name},${company},${email},${contactNumber},${city},${state},${country},${product},${status},${spam},${date},${ip}\n`;
    });
    ...
    link.setAttribute("download", "catalogue_submissions.csv");
    ...
    <div className="min-h-screen bg-gray-50 p-4 sm:p-6">
      <header className="mb-6 sm:mb-8">
        <div className="text-center">
          ...
          <p className="text-sm sm:text-base text-gray-600">Manage catalog download requests</p>
        </div>
      </header>
                    ...
                    <div className="space-y-2">
                      ...
                      {submission.companyName && (
                        <div>
                          ...
                          <p className="text-sm text-gray-900">{submission.companyName}</p>
                        </div>
                      )}
                      
                      <div>
                        <span className="text-xs font-medium text-gray-500 uppercase tracking-wider">Product</span>
                        <p className="text-sm text-gray-900">{submission.productName || "Manufacturing Range"}</p>
                      </div>
                      ...
                      <div className="mt-4 pt-4 border-t border-gray-200">
                        <div className="space-y-3">
                          ...
                          {submission.contactNumber && (
                            <div>
                              <span className="text-xs font-medium text-gray-500 uppercase tracking-wider">Contact Number</span>
                              <p className="text-sm text-gray-900">{submission.contactNumber}</p>
                            </div>
                          )}
            ...
            <thead className="bg-gray-50">
              <tr>
                ...
                {visibleColumns.company && (
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100" onClick={() => handleSort("companyName")}>
                    <div className="flex items-center">
                      COMPANY
                      <span className={`ml-1 ${sortField === "companyName" ? (sortDirection === "asc" ? "text-blue-600" : "text-blue-600") : "text-gray-400"}`}>
                        {sortField === "companyName" ? (sortDirection === "asc" ? "↑" : "↓") : "↕"}
                      </span>
                    </div>
                  </th>
                )}
                ...
                {visibleColumns.contactNumber && (
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100" onClick={() => handleSort("contactNumber")}>
                    <div className="flex items-center">
                      CONTACT
                      <span className={`ml-1 ${sortField === "contactNumber" ? (sortDirection === "asc" ? "text-blue-600" : "text-blue-600") : "text-gray-400"}`}>
                ...
                {visibleColumns.product && (
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100" onClick={() => handleSort("productName")}>
                    <div className="flex items-center">
                      PRODUCT
                      <span className={`ml-1 ${sortField === "productName" ? (sortDirection === "asc" ? "text-blue-600" : "text-blue-600") : "text-gray-400"}`}>
                  ...
                  return (
                    <React.Fragment key={submission._id || index}>
                      <tr className={`${expandedRow === (submission._id || index) ? "bg-blue-50" : ""} hover:bg-gray-50`}>
                        ...
                        {visibleColumns.company && <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{submission.companyName}</td>}
                        ...
                        {visibleColumns.product && <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{submission.productName || "Manufacturing Range"}</td>}
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                          <select
                            value={submission.followupStatus || "Pending"}
                            onChange={(e) => updateStatus(submission._id, e.target.value)}
                            ...
                            <div className="p-6">
                              ...
                              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                ...
                                <div className="bg-white p-3 rounded-lg border">
                                  ...
                                  <div className="text-sm text-gray-900 mt-1">{submission.companyName}</div>
                                </div>
                                <div className="bg-white p-3 rounded-lg border">
                                ...
                                <div className="bg-white p-3 rounded-lg border">
                                  <div className="text-xs font-medium text-gray-500 uppercase tracking-wider">Contact No</div>
                                  <div className="text-sm text-gray-900 mt-1 flex items-center space-x-2">
                                    <svg className="w-4 h-4 text-gray-500" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />
                                    </svg>
                                    <span>{submission.contactNumber}</span>
                                ...
                                <div className="bg-white p-3 rounded-lg border">
                                  <div className="text-xs font-medium text-gray-500 uppercase tracking-wider">City</div>
                                  <div className="text-sm text-gray-900 mt-1">{submission.city}</div>
                                </div>
                                <div className="bg-white p-3 rounded-lg border">
                                  <div className="text-xs font-medium text-gray-500 uppercase tracking-wider">State</div>
                                  <div className="text-sm text-gray-900 mt-1">{submission.state}</div>
                                </div>
                                <div className="bg-white p-3 rounded-lg border">
                                  <div className="text-xs font-medium text-gray-500 uppercase tracking-wider">Country</div>
                                  <div className="text-sm text-gray-900 mt-1">{submission.country}</div>
                                </div>
                                <div className="bg-white p-3 rounded-lg border">
                                  <div className="text-xs font-medium text-gray-500 uppercase tracking-wider">Product</div>
                                  <div className="text-sm text-gray-900 mt-1">{submission.productName || "Manufacturing Range"}</div>
                                </div>
                                <div className="bg-white p-3 rounded-lg border">
                                  <div className="text-xs font-medium text-gray-500 uppercase tracking-wider">URL</div>
                                  <div className="text-sm text-gray-900 mt-1">
                                    <a href={submission.url} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:text-blue-800 underline">
                                      {submission.url}
                                    </a>
                                  </div>
                                </div>
...
export default CatalogueSubmissions;
